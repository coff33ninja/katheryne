...
export class GenshinDevClient {
    ...
    async getCharacter(characterId: string): Promise<DetailedCharacter> {
        const response = await axios.get<DetailedCharacter>(`${this.baseUrl}/characters/${characterId}`);
        return response.data;
    }
    ...
    async getWeapon(weaponId: string): Promise<Weapon> {
        const response = await axios.get<Weapon>(`${this.baseUrl}/weapons/${weaponId}`);
        return response.data;
    }
}