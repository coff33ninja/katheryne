...
class GenshinAssistantDataset(Dataset):
    ...
    def __init__(self, data_path: Path):
        ...
        print(f"Dataset loaded: {len(self.queries)} samples, vocabulary size: {self.vocab_size}")
    ...
    def _text_to_tensor(self, text: str, max_length: int = 128) -> torch.Tensor:
...
class GenshinAssistant(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        hidden = hidden.view(2, 2, batch_size, -1)  # [num_layers, num_directions, batch, hidden]
        hidden = hidden.transpose(1, 2).contiguous()  # [num_layers, batch, num_directions, hidden]
        hidden = hidden.view(2, batch_size, -1)  # [num_layers, batch, hidden*num_directions]
        ...
        for t in range(tgt_len - 1):  # -1 because we don't need to generate after last token
            # Calculate attention weights
...
class GenshinAssistantTrainer:
    ...
    def train(self, epochs: int = 100, batch_size: int = 16, learning_rate: float = 0.001):
    ...
    def generate_response(self, query: str, max_length: int = 128) -> str:
        ...
        with torch.no_grad():
            # Encode query
            src_embedded = self.model.embedding(query_tensor)
            ...
            for _ in range(max_length):
                ...
                token_embedded = self.model.embedding(current_token)
                
                # Calculate attention
                attn_weights = self.model.attention_weights(decoder_hidden[-1], encoder_outputs)
                context = torch.bmm(attn_weights.transpose(1, 2), encoder_outputs)
